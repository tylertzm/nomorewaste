...
export default function App() {
    ...
    const processReceipt = async (e) => {
        ...
        reader.onload = async () => {
            ...
            const systemPrompt = `You are a grocery receipt parser. Extract items into a JSON array. For each item include: 'name', 'price' (number), 'category' (Produce, Dairy, Meat, Beverage, Pantry, Bakery, Frozen, Other), and 'expiry' (estimate YYYY-MM-DD based on today's date).`;
            ...
            try {
                ...
                setDraftItems(parsed.map(i => ({ ...i, id: Math.random().toString(36).substr(2, 9) })));
                setModalStep('verify');
            } catch (error) {
        ...
        <div className="min-h-screen bg-slate-50 text-slate-900 font-sans selection:bg-emerald-100">
                ...
                {activeTab === 'fridge' ? (
                    <div className="space-y-4">
                        {items.length === 0 ? (
                            ...
                            items.map(item => {
                                const isExpired = new Date(item.expiry) < new Date(); const daysLeft = Math.ceil((new Date(item.expiry) - new
                                    Date()) / (1000 * 60 * 60 * 24)); return (<div key={item.id}
                    ...
                    ${CATEGORIES[item.category]?.color || CATEGORIES.Other.color}`}>
                                        ...
                                        <div className="flex gap-2">
                                            <button onClick={() => markConsumed(item.id)} className="p-2 bg-emerald-50 text-emerald-600
                        rounded-lg active:scale-90 transition-transform">
                                                <CheckCircle2 size={18} />
                                            </button>
                                            <button onClick={() => markWasted(item.id)} className="p-2 bg-red-50 text-red-500 rounded-lg
            ...
            <div className="fixed bottom-8 left-0 right-0 px-6 max-w-md mx-auto z-40">
                <button onClick={() => setIsModalOpen(true)}
                    className="w-full bg-slate-900 text-white py-4 rounded-2xl shadow-xl shadow-slate-200 font-black flex
        items-center justify-center gap-2 hover:bg-slate-800 active:scale-95 transition-all"
                >
                    <Camera size={20} />
                    SCAN RECEIPT
                </button>
            ...
            {isModalOpen && (
                <div className="fixed inset-0 z-50 flex items-end justify-center bg-slate-900/60 backdrop-blur-sm p-4">
                    <div
                        className="bg-white w-full max-w-md rounded-[2.5rem] p-8 max-h-[90vh] flex flex-col shadow-2xl animate-in slide-in-from-bottom-10">
                        ...
                        <div className="flex-1 overflow-y-auto custom-scrollbar">
                            {modalStep === 'upload' && (
                                <div className="text-center py-10">
                                    ...
                                    <p className="text-slate-500 text-sm mb-8 px-4">Upload a clear photo of your receipt. Gemini will
                                        automatically extract names, prices, and expiry dates.</p>
                                    <label
                                        className="block w-full py-4 bg-emerald-600 text-white rounded-2xl font-black cursor-pointer text-center">
                                        CHOOSE PHOTO
                            ...
                            {modalStep === 'verify' && (
                                <div className="space-y-4">
                                        ...
                                        <div key={item.id} className="bg-slate-50 p-4 rounded-2xl space-y-3">
                                            ...
                                            <div className="grid grid-cols-2 gap-3">
                                                <div>
                                                    ...
                                                    <input type="number" value={item.price} onChange={(e) => updateDraft(item.id, 'price',
                                                ...
                                                <div>
                                                    ...
                                                    <input type="date" value={item.expiry} onChange={(e) => updateDraft(item.id, 'expiry',
                                                        e.target.value)}
                                                        className="w-full bg-white border border-slate-200 p-2 rounded-lg text-xs font-bold"
                                                    />
                                                </div>
                                    ...
                                    <button onClick={() => setDraftItems(prev => [...prev, {
                                        id: Math.random().toString(), name: 'New Item',
                                        price: 0, category: 'Other', expiry: new Date().toISOString().split('T')[0]
                                    }])}
                                        className="w-full py-3 border-2 border-dashed border-slate-200 rounded-2xl text-slate-400 font-bold
